...
module("Acceptance | collection", function (hooks) {
  ...
  test("collection works as expected", async function (assert) {
    ...
    assert.deepEqual(
      list.items
        .findAll((i) => i.isActive || i.text === "Foo")
        .map((i) => i.text),
      ["Foo", "Bar"]
    );
    assert.deepEqual(
      list.items.findAll({ text: "Bar", isActive: true }).map((i) => i.text),
      ["Bar"]
    );
    assert.deepEqual(
      list.items.findAll({ text: "Foo", isActive: true }).map((i) => i.text),
      []
    );

    assert.equal(list.items.findOne((i) => i.isActive).text, "Bar");
    assert.equal(
      list.items.findOne({ text: "Bar", isActive: true }).text,
      "Bar"
    );
    assert.throws(
      () => list.items.findOne({ text: "Foo", isActive: true }),
      /Expected at most one result.*'findOne' query in 'items' collection.*but found 0.*using query.*isActive: true/
    );
    assert.throws(
      () => list.items.findOne((i) => i.isActive || i.text === "Foo"),
      /Expected at most one result.*'findOne' query in 'items' collection.*but found 2/
    );
  });
